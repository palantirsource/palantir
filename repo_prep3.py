...
class Generator:
    ...
    def _generate_md5_file(self):
        try:
            ...
            m = hashlib.md5(open(self.addons_xml, 'rb').read()).hexdigest()

            # save file
            self._save_file(m.encode(), self.addons_xml_md5)
...
class Compressor:
    ...
    def _create_compressed_addon_release(self):
        ...
        for the_file in self.addon_folder_contents:
            ...
            elif not (('addon.xml' in the_file) or ('changelog' in the_file) or ('fanart' in the_file) or ('icon' in the_file) or (zipname in the_file)):
                os.remove(the_path)

            # tag the changelog with '-x.x.x' release number
            elif 'changelog' in the_file:  # hangelog so that it is detected irrespective of whether C is capitalised
    ...
    def _read_version_number(self):
        ...
        headers = re.findall(r"\<addon id\=(.+?)>", self.addon_xml, re.DOTALL)
        ...
        for header in headers:
            ...
            if self.addon_name in header:
                ...
                self.addon_version_number = (re.findall(r"version\=(.+?) ", header)[0]).strip()
